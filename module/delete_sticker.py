...
from utils import async_get_sticker_pack_name, filter_inline_query_results, get_auto_indexed_packages, get_sticker_id, is_admin
    ...
    with DBSession.begin() as session:
        ...
        if 'https://t.me/addstickers/' in query:
            ...
            stmt = select(AutoIndexSticker).filter(
                ...
                AutoIndexSticker.set_name == pack_name
            )
        ...
        # 删除单张贴纸
            ...
            if get_auto_indexed_packages(result.set_name, chosen.from_user.id):
                ...
                await client.edit_inline_reply_markup(chosen.inline_message_id, reply_markup=button)
            else:
                session.delete(result)
        return
